...
document.getElementById("formAdocao").addEventListener("submit", function (e) {
    ...
    if (moradia === "apartamento" && quintalRadio.value === "sim") {
        return alert("Inconsistência: quem mora em apartamento não pode indicar que possui quintal.");
    }
    ...
    if (moradia === "casa" && quintalRadio.value === "sim" && quintalSeguroRadio.value === "nao") {
        alert("Atenção: a ONG recomenda que o quintal seja cercado para a segurança do animal.");
    }
 
    if (cpfsCadastrados.includes(cpf)) {
        return alert("CPF já cadastrado no sistema, não é possível realizar novo cadastro.");
    }
    ...
    if (tevePetRadio.value === "nao") {
        avisos += `<div class="aviso">🐾 Como você nunca teve um pet antes, a ONG poderá realizar acompanhamento nos primeiros meses após a adoção.</div>`;
    }
 
    if (horasSozinho > 8) {
        avisos += `<div class="aviso"> Atenção: o animal ficará mais de 8h sozinho, certifique-se de que ele terá água, comida e um ambiente seguro.</div>`;
    }
